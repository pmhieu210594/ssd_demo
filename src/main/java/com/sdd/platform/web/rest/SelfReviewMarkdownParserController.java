...
/**
 * Parser endpoint for {@code self-review.md}.
 *
 * The file endpoint is intentionally restricted to docs/changes/<TICKET>/self-review.md
 * so it cannot be used as a generic file-read primitive.
 */
@RestController
@RequestMapping("/api/v1/markdown-parser/self-review")
public class SelfReviewMarkdownParserController {

    private static final List<String> ALLOWED_EXTENSIONS = List.of(".md", ".markdown");

    private final SelfReviewMarkdownParser parser;

    public SelfReviewMarkdownParserController(SelfReviewMarkdownParser parser) {
        this.parser = parser;
    }
    ...
    public Map<String, Object> parseFile(@RequestBody ParseFileRequest req) {
        ...
        if (!normalizedPath.contains("/changes/") || !normalizedPath.endsWith("/self-review.md")) {
            throw new IllegalArgumentException("Only <root>/changes/<TICKET>/self-review.md can be parsed");
        }
    ...
    private Map<String, Object> toResponse(ParsedArtifact parsed) {
        ...
        response.put("freeTextSections", parsed.freeTextSections());
        response.put("tables", parsed.tables());
        response.put("tableSections", parsed.tableSections());
