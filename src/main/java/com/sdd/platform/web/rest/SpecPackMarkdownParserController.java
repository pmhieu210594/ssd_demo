...
import com.sdd.platform.domain.service.markdown.specpack.SpecPackMarkdownParser.ParsedArtifact;
...
/**
 * Parser endpoint for `spec-pack.md`.
 *
 * The file endpoint is intentionally restricted to docs/changes/<TICKET>/spec-pack.md
 * so it cannot be used as a generic file-read primitive.
 */
@RestController
@RequestMapping("/api/v1/markdown-parser/spec-pack")
public class SpecPackMarkdownParserController {

    private static final List<String> ALLOWED_EXTENSIONS = List.of(".md", ".markdown");

    private final SpecPackMarkdownParser parser;

    public SpecPackMarkdownParserController(SpecPackMarkdownParser parser) {
        this.parser = parser;
    }

    @PostMapping("/parse-markdown")
    public Map<String, Object> parseInline(@RequestBody ParseRequest req) {
        ParsedArtifact parsed = parser.parse(req.content() != null ? req.content() : "", req.sourcePath(), req.parseMode());
    ...
    public Map<String, Object> parseFile(@RequestBody ParseFileRequest req) {
        ...
        // Guard 2: the path must stay inside docs/changes/**/spec-pack.md.
        String normalizedPath = path.toString().replace('\\', '/');
        if (!normalizedPath.contains("/changes/") || !normalizedPath.endsWith("/spec-pack.md")) {
            throw new IllegalArgumentException("Only <root>/changes/<TICKET>/spec-pack.md can be parsed");
        }
