-- Artifact snapshot schema_version: migrate from text to integer and expose it in the inventory view.

ALTER TABLE tbl_fact_artifact_snapshot
    ALTER COLUMN schema_version TYPE INTEGER
    USING CASE
        WHEN schema_version IS NULL THEN NULL
        ELSE COALESCE(
            NULLIF(regexp_replace(schema_version::text, '[^0-9]', '', 'g'), '')::INTEGER,
            1
        )
    END;
