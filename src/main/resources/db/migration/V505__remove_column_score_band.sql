-- remove score_band in view tbl_fact_ticket_dashboard_snapshot
-- Step 1: remove view old
DROP VIEW IF EXISTS tbl_fact_ticket_dashboard_snapshot;
-- Step 2: create view
CREATE VIEW tbl_fact_ticket_dashboard_snapshot AS
...
ticket_repo AS (
    ...
    ORDER BY ticket_id, collected_at DESC, created_at DESC, artifact_snapshot_id DESC
),
latest_ticket_ci_repo AS (
    SELECT DISTINCT ON (ticket_id)
        ticket_id,
        repository_id
    FROM tbl_fact_ci_run
    WHERE ticket_id IS NOT NULL
    ORDER BY ticket_id, COALESCE(updated_at, created_at, collected_at, now()) DESC, ci_run_id DESC
),
latest_score AS (
    SELECT DISTINCT ON (ticket_id)
        ticket_id,
...
open_issues AS (
    SELECT
        ti.ticket_id,
        COUNT(*) FILTER (WHERE UPPER(ti.issue_status) = 'OPEN') AS open_issue_count
...
SELECT
    ...
    t.project_id,
    p.project_alias,
    COALESCE(r.repository_id, pr.repository_id) AS repository_id,
    COALESCE(r.repo_name_masked, pr.repo_name_masked) AS repository_name,
    ...
    COALESCE(
        ...
        NULLIF(BTRIM(mp.pseudonym), ''),
        'Unknown'
    )::varchar AS owner_display,
    ...
    LOWER(CONCAT_WS(
        ...
        COALESCE(t.title, ''),
        p.project_alias,
        COALESCE(r.repo_name_masked, pr.repo_name_masked),
        COALESCE(
            NULLIF(BTRIM(lpr.author_display_name), ''),
            NULLIF(BTRIM(pr_a.fullname), ''),
            NULLIF(BTRIM(t.created_by), ''),
...
LEFT JOIN latest_ticket_ci_repo lcr ON lcr.ticket_id = t.ticket_id
LEFT JOIN primary_repo pr ON pr.project_id = p.project_id
LEFT JOIN tbl_dim_repository r ON r.repository_id = COALESCE(tr.repository_id, lcr.repository_id, pr.repository_id)
...
--drop column score_band
ALTER TABLE tbl_fact_evidence_quality_score DROP COLUMN IF EXISTS score_band;