...
**Verdict: Request changes.** → **Cập nhật 2026-08-21 (sau triage con người): tất cả 5 finding đã
có quyết định.** Xem bảng dưới và [§3](#3-tổng-hợp-findings).

0 Blocker · 2 Major · 3 Minor. (Định nghĩa severity: xem [§9 Phụ lục](#9-phụ-lục--quy-ước).)

**Kết quả triage con người (2026-08-21):**

1. [F1](#f1) (Major, Correctness) — **Đã xử lý**: người dùng đã đọc lại patch (thêm điều kiện `ON
   d.document_update_at >= d.document_create_at`) và **xác nhận đúng**.
2. [F2](#f2) (Major, quy trình/audit) — **Đã xử lý (waived)**: người dùng **xác nhận PASS** —
   chấp nhận `self-review.md` làm nguồn duy nhất cho ticket này, không yêu cầu điền lại
   `review-checklist.md`.
3. [F3](#f3) (Minor, log noise) — **Không fix**: người dùng xác nhận bỏ qua.
4. [F4](#f4) (Minor, N+1 write) — **Đã duyệt đề xuất sửa**: người dùng xác nhận áp dụng
   `batchUpdate` — **chưa implement**, cần 1 lượt code fix riêng trước khi đóng hẳn.
5. [F5](#f5) (Minor, log thiếu nguyên nhân) — **Không fix**: người dùng xác nhận bỏ qua.

Không phát hiện AC nào "chưa đạt hoàn toàn" ngoài F1 (đã xử lý) — xem [§6](#6-traceability--ac-chưa-đạt).
...
| --- | --- | --- | --- |
| Implement fix [F4](#f4) (`batchUpdate`) | Đã được duyệt đề xuất sửa 2026-08-21, chưa có code — cần 1 lượt dev + test mới (TC2 ở [§7](#7-đề-xuất-test-cases-bổ-sung)) | Dev | *cần điền* |
| Chạy Independent Review này lại **trên Postgres thật** một lần trước khi coi AC-2/AC-6 là đã verify runtime | Vẫn là khoảng trống đã biết từ `test-results.md §8`, review này không lấp được (không có Postgres trong môi trường review) | QA/Dev trước deploy | *cần điền* |
...
| --- | --------- | ----------------- | --------------------------------------------- | ------------ | ----------- | ---------- |
| F1  | Major | Correctness | [1 file có `update_date < create_date` trong 1 phase có ≥2 file bị netted (cộng bù trừ) vào tổng thay vì bị loại trừ, làm tổng Dwell Time sai lệch một cách âm thầm](#f1) | AC-2 | Người dùng | Đã xử lý (xác nhận 2026-08-21) — xem [Cập nhật xử lý F1](#f1-fix) |
| F2  | Major | Missing tests | [`review-checklist.md` chưa được điền thật (9/9 dòng §1 vẫn "TODO", 99/99 checkbox §2-10 chưa tick) dù `self-review.md` đã tự chấm PASS cho các mục tương ứng](#f2) | N/A (process/audit) | Người dùng | Đã xử lý (waived) — người dùng xác nhận PASS 2026-08-21: chấp nhận `self-review.md` làm nguồn duy nhất cho ticket này, không yêu cầu điền lại `review-checklist.md` |
| F3  | Minor | Robustness | [WARN log cho tổng âm bị tính lại và log lại mỗi lần mở Ticket Detail (đọc), không chỉ 1 lần lúc quét (ghi) — có thể gây log noise nếu ticket lỗi dữ liệu bị xem nhiều lần](#f3) | spec-pack §12 (Operation/Logging) | Người dùng | Không fix (chấp nhận rủi ro, xác nhận 2026-08-21) |
| F4  | Minor | Correctness | [`upsertArtifactDocumentDates` ghi từng dòng bằng vòng lặp `jdbc.update` tuần tự thay vì 1 lệnh `batchUpdate` — N round-trip DB không cần thiết mỗi lần quét (N ≤ 8)](#f4) | N/A (impl-plan § Phương châm data/DB/query) | Người dùng | Đã duyệt đề xuất sửa (2026-08-21) — **chưa implement**, chờ dev áp dụng `batchUpdate` |
| F5  | Minor | Robustness | [`ArtifactDocumentDateService.extract` nuốt exception khi đọc/parse 1 file mà không log `ex.getMessage()` — chỉ log nhãn cứng `"content"`, mất thông tin nguyên nhân thật (IO lỗi, encoding, v.v.) dù không vi phạm quy tắc "không log nội dung file"](#f5) | Gate #5 (impl-plan.md, log policy) | Người dùng | Không fix (chấp nhận rủi ro, xác nhận 2026-08-21) |
