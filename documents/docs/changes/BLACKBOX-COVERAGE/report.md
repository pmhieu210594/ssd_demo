...
- Backend: thêm schema additive `V516` (4 bảng blackbox), port/adapter persistence riêng, scanner parse `blackbox-testcases.md`, service map `test-results.md` → `PASS/FAIL/SKIP/NOT_RUN`.
...
| # | Khu vực | Chi tiết |
| --- | --- | --- |
| 1 | Các tệp đã thay đổi | Migration `V516__blackbox_coverage_schema.sql`; port `BlackboxCoveragePersistencePort`; adapter `BlackboxCoverageJdbcAdapter`; `ArtifactScannerService`; `TestResultsParseService`; `QaDashboardModels`, `QaDashboardRepositoryPort`, `QaDashboardJdbcAdapter`, `QaDashboardService`. Danh sách đầy đủ + lý do: [impl-plan.md §4](impl-plan.md) |
...
| BE API IT | `mvn -Dit.test=QaDashboardApiIntegrationTest,... integration-test` | PASS | 5 tests, 0 fail, qua Postgres local thật + Flyway V516 |
...
3. Vì migration `V516` là additive, việc revert không yêu cầu rollback dữ liệu AC-Test Coverage hiện hữu.
