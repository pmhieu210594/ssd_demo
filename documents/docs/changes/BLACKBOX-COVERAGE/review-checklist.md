...
|---|---|---|---|---|
| DD-1 | Migration `V516` chỉ thêm bảng mới, không sửa bảng cũ | Blocker | AC-7 | PASS — xác nhận qua `\d tbl_fact_blackbox_*` trên DB thật, không có ALTER lên bảng cũ |
...
| # | Check | Severity | AC ref | Result |
|---|---|---|---|---|
| PERF-1 | Index đủ cho truy vấn theo ticket trên 4 bảng blackbox mới | Major | AC-3,4 | PASS (review tĩnh) — `V516` có index trên `ticket_id` cho cả 4 bảng; chưa đo runtime trên dữ liệu lớn |
