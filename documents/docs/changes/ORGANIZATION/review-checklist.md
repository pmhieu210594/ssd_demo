...
|---|---|---|---|---|---|
| GEN-001 | Major | `version` is required for edit/delete operations that need optimistic locking. | Missing/non-numeric/stale version is rejected safely. | PASS | AC-ORGANIZATION-7, 8, 13 |
| GEN-002 | Major | Pagination parameters, if implemented, are validated. | Invalid page/size cannot cause server error or excessive query. | PASS | Match existing API convention. |
| GEN-003 | Major | Empty string/null handling for code/name/description is explicit. | Required fields reject blank after trim; optional fields are normalized consistently. | PASS | FE and BE should match. |
| GEN-004 | Minor | Numeric overflow/underflow for `version` is considered. | Very large/invalid numeric input does not crash BE. | PASS | Validation or binding error returns 400. |

### 2.2. Character Type / Encoding / Locale

| ID | Severity | Review item | Expected | Result | Notes |
|---|---|---|---|---|---|
| GEN-005 | Major | Code/name/description max lengths follow spec. | Code max 50, name max 255, description max 500 are enforced. | PASS | DB `TEXT` does not remove API validation requirement. |
| GEN-006 | Major | Trim and blank rules are consistent. | Leading/trailing spaces do not bypass required/duplicate checks. | PASS | Duplicate checks should use normalized values. |
| GEN-007 | Major | Locale JSON remains valid UTF-8. | English/Japanese/Vietnamese messages render without mojibake. | PASS | Encoding must remain UTF-8. |
| GEN-008 | Major | Message keys are normalized. | Use `Pages.Organization.*.*`; do not introduce `Name.Eequired`, lowercase variants, or `Organizationc`. | PASS | Resolved OI-P3-ORG-004. |
| GEN-009 | Minor | Full-width/half-width/emoji/surrogate pairs are considered. | Inputs are stored/displayed safely or rejected per validation. | PASS | Especially Organization name/description. |

### 2.3. Literal / Magic Number

| ID | Severity | Review item | Expected | Result | Notes |
|---|---|---|---|---|---|
| GEN-010 | Major | Status values are centralized or clearly mapped. | `ACTIVE`, `DELETED`, and `All` filter semantics are not scattered as magic literals. | PASS | `All` is query/UI only, not persisted. |
| GEN-011 | Major | API paths are defined consistently. | Use `/api/v1/organizations`; normal edit uses `PUT /api/v1/organizations/{id}`. | PASS | Resolved OI-P3-ORG-003. |
| GEN-012 | Minor | Max length values are constants or shared validation metadata where practical. | No inconsistent code/name/description limits between FE and BE. | PASS | 50/255/500. |

### 2.4. Operation / Maintainability

| ID | Severity | Review item | Expected | Result | Notes |
|---|---|---|---|---|---|
| GEN-013 | Major | Implementation follows existing architecture. | Controller has no business logic; use case/service and repository/adapter are separated. | PASS | Hexagonal architecture. |
| GEN-014 | Major | Double-submit/update conflict is safe. | Version conflict prevents duplicate overwrite/delete. | PASS | AC-ORGANIZATION-13. |
| GEN-015 | Major | Configuration is not hard-coded. | No environment-specific constants in source. | PASS | Especially routes/API base already follow existing helper. |
| GEN-016 | Major | No out-of-scope implementation is added. | Customer, connector ingestion, and dedicated audit log are not implemented by this ticket. | PASS | Scope control. |
...
| ID | Severity | Review item | Expected | Result | Notes |
|---|---|---|---|---|---|
| SEC-001 | Blocker | BE enforces ADMIN-only access. | FE-only guard is not trusted as the only protection. | PASS | AC-ORGANIZATION-11. |
| SEC-002 | Blocker | Unauthorized and unauthenticated statuses are correct. | Unauthenticated is `401`; non-ADMIN is `403`. | PASS | Spring Security behavior respected. |
| SEC-003 | Major | Errors do not leak stack traces or internals. | Client receives stable error/status/message key/traceId only. | PASS | Security and UX. |
| SEC-004 | Major | Logs do not expose secrets/tokens/OAuth raw attributes. | Only operationally necessary Organization data is logged. | PASS | Privacy. |
| SEC-005 | Major | CSRF/session-cookie assumptions are not broken. | Existing auth flow remains compatible. | PASS | Current session auth. |
| SEC-006 | Major | Organization name privacy semantics are unchanged. | Existing `name_masked` column is not renamed/reinterpreted unexpectedly. | PASS | Schema convention. |
| SEC-007 | Major | Non-ADMIN cannot bypass by direct URL/API manipulation. | Route guard and BE authorization both work. | PASS | Defense in depth. |
...
| ID | Severity | Review item | Expected | Result | Notes |
|---|---|---|---|---|---|
| TEST-001 | Major | Active default list is verified. | ADMIN sees active Organizations by default. | PASS | AC-ORGANIZATION-1. |
| TEST-002 | Major | Search behavior is verified. | Code/name matching and non-matching records tested. | PASS | AC-ORGANIZATION-2. |
| TEST-003 | Major | Status filters are verified. | All/Active/Deleted cases tested. | PASS | AC-ORGANIZATION-3. |
| TEST-004 | Major | Create success is verified. | Valid unique code/name creates row and returns to list. | PASS | AC-ORGANIZATION-4. |
| TEST-005 | Blocker | Duplicate code behavior is verified. | Active duplicate returns 409; soft-deleted reuse allowed. | PASS | AC-ORGANIZATION-5. |
| TEST-006 | Blocker | Duplicate name behavior is verified. | Active duplicate returns 409; soft-deleted reuse allowed. | PASS | AC-ORGANIZATION-6. |
| TEST-007 | Blocker | Edit code with version is verified. | Valid update increments version; stale/duplicate rejected. | PASS | AC-ORGANIZATION-7. |
| TEST-008 | Blocker | Edit name with version is verified. | Valid update increments version; stale/duplicate rejected. | PASS | AC-ORGANIZATION-8. |
| TEST-009 | Blocker | Soft delete behavior is verified. | Row remains in DB and is marked deleted. | PASS | AC-ORGANIZATION-9. |
| TEST-010 | Major | Deleted read-only behavior is verified. | Deleted hidden by default and read-only when filtered. | PASS | AC-ORGANIZATION-10. |
| TEST-011 | Blocker | Authorization behavior is verified. | FE non-ADMIN logout/redirect and BE direct API 403. | PASS | AC-ORGANIZATION-11. |
| TEST-012 | Blocker | Stale version conflict is verified. | 409 with `Pages.Organization.Conflict.Version`; no overwrite/delete. | PASS | AC-ORGANIZATION-13. |
| TEST-013 | Major | Migration/backfill is verified. | Required columns/indexes exist; data preserved. | PASS | OI-P3-ORG-001. |
| TEST-014 | Major | i18n rendering is verified. | `en`, `ja`, `vi` keys render correctly. | PASS | OI-P3-ORG-004. |
| TEST-015 | Accepted Risk | FE automated tests are deferred if still approved. | Deferral is recorded in self-review/test-results/report. | PASS | User decision; not accidental omission. |
| TEST-016 | Question | DB integration test setup is decided later. | Testcontainers/local DB/manual verification decision is recorded. | PASS | OI-P3-ORG-007 deferred. |
