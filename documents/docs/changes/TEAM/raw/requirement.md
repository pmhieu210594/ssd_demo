...
- Create new tables only when truly necessary.
- A member's role in a Team is data that belongs to the Team-Member relationship, so an intermediate table is required.
- The Teams screen does not manage assigning Teams to Projects.
...
| 3 | Use the existing `tbl_dim_role` for a member's role in a Team |
| 4 | One member can belong to multiple Teams |
| 5 | Within one Team, one member can have only one active role |
| 6 | Deleting a Team means soft delete/inactive |
| 7 | Removing a member from a Team means making the membership inactive |
...
- In this screen, Team no longer directly depends on Project.

### 3.2 `tbl_dim_member_pseudonym`

Use this as the source for selecting members when adding a member to a Team.

Do not use `team_id` / `role_id` on this table to represent the new membership.

### 3.3 `tbl_dim_role`

Use this as the role master for a member's role in a Team.

### 3.4 `tbl_auth_member_project_role`

Do not use this for Team membership because it represents an authorization relationship within the Project scope, not the member relationship within a Team.
...
If any foreign key/index related to `project_id` currently exists, drop the constraint/index before dropping the column.

Example constraint name that needs to be checked in the current DB/migration:
...
If some columns already exist, only add the missing columns and do not recreate the table.

### 4.3 Unique Constraint/Index

Team Code can be edited after creation, but it must be unique among active Teams.
...
### 5.1 Reason for the Intermediate Table

`tbl_team_member` needs to be created because:

- One Team has many members.
- One member can belong to multiple Teams.
- Within one Team, the role is an attribute of the Team-Member relationship.
- Within the same Team, one member can have only one active role.
- The role in a Team should not be stored in the member master.
...
| TEAM-RULE-006 | Deleting a Team must inactive all active memberships of that Team within the same transaction |
...
| TEAM-MEMBER-RULE-008 | Updating the role only updates `tbl_team_member.role_id`; it does not update the member master |
...
|---|---|
| TEAM_CODE_DUPLICATED | Violates the unique active team_code constraint |
| TEAM_NOT_FOUND | Team not found |
| TEAM_ALREADY_INACTIVE | Team is no longer active |
| TEAM_MEMBER_ALREADY_EXISTS | Violates the unique active `(team_id, member_key)` constraint |
...
- If assigning Teams to Projects is required in the future, create a separate ticket/screen and design a separate relationship.
