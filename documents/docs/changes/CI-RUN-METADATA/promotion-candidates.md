...
- Check the existing architecture/standards first, then promote only the smallest reusable pattern.
- Add the failure mode before implementation if the bug could recur across tickets.
- Keep connector diagnostics in knowledge or architecture unless the same rule appears in multiple tickets.
- Do not promote one-off delivery gaps into permanent guidance.
