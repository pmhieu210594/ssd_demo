...
|---|---|---|---|
| AC-1 | Anonymous user can open `/:lang/login` and see username, password, password toggle, and login button. | Yes | FE/E2E. |
| AC-2 | Empty username prevents login API submission. | Yes | FE. |
| AC-3 | Empty password prevents login API submission. | Yes | FE. |
| AC-4 | Password is masked by default. | Yes | FE/BB. |
| AC-5 | Pending login disables duplicate submit. | Yes | FE/BB. |
| AC-6 | Valid active credentials return token response and user summary. | Yes | BE/IT/E2E. |
| AC-7 | Backend trims username before lookup. | Yes | BE. |
| AC-8 | Unknown username and wrong password show generic invalid-credential behavior. | Yes | FE/BE/BB. |
| AC-9 | Inactive account is rejected safely with no token. | Yes | BE/IT. |
| AC-10 | Inactive account is rejected safely with no token issued. | Yes | BE/BB. |
| AC-11 | Successful login revokes previous active token sessions. | Yes | BE. |
| AC-12 | Admin login redirects to `/:lang/admin`. | Yes | FE/E2E. |
| AC-13 | Non-admin login redirects to `/:lang/`. | Yes | FE/E2E. |
| AC-14 | Authenticated user opening login is redirected by role. | Yes | FE. |
| AC-15 | Missing/invalid/expired/revoked token is rejected and FE becomes anonymous. | Yes | FE/BE/IT. |
| AC-16 | `/api/v1/auth/me` and `/api/v1/me` return compatible current-user payloads. | Yes | IT/contract. |
| AC-17 | Logout revokes server token sessions when principal exists and returns 204. | Yes | BE/IT. |
| AC-18 | FE logout clears local access/refresh tokens and auth query state even if API fails. | Yes | FE. |
| AC-19 | Auth errors never expose passwords, hashes, tokens, stack traces, SQL details, or account-existence hints. | Yes | Security review/BB. |
| AC-20 | Out-of-scope auth features are not active LOGIN paths. | Yes | FE/BB. |
