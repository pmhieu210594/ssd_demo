...
| Host type | `host_type` | `spec-pack.md`, `Repository.java` | Existing enum values: `GITHUB`, `GITLAB`, `LOCAL`. |
...
| Code Value | `GITHUB`, `GITLAB`, `LOCAL`, `ACTIVE`, `DELETED` | Domain model + migration/spec | Use constants or enum mapping, not magic strings. |
