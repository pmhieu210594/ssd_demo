...
## Human Clarifications Received on 2026-06-12

- Only `ADMIN` can access the User Management screen and perform account operations.
- Team assignment is out of scope; no team input, filter, or assignment is allowed.
- `team_id = NULL` is intentional and accepted for this release.
- Hard delete is not allowed.
- Deactivate, reactivate, and reset password are supported.
- The last active Admin account must not be deactivated or downgraded.
- Passwords are stored as bcrypt hashes and raw passwords must never be returned or logged.

## Phase 3 Carry-over After Clarification

- Keep FE and BE password validation aligned before promoting implementation.
- Preserve the `tbl_auth_user_account` and `tbl_dim_member_pseudonym` split in contracts and tests.
- Keep `team_id = NULL` as a hard contract in the mapper and validation layer.
- Retain the Admin-only guard and last-Admin protection in route, API, and test coverage.
